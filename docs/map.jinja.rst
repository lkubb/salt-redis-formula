...
We will see a quick setup to configure the ``redis`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``redis`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``redis`` formulas under ``/srv/salt/redis/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/redis/parameters/dns:domain/
    ln -s dns:domain /srv/salt/redis/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/redis/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/redis/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``redis`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``redis`` formulas under ``/srv/salt/redis/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/redis/parameters/roles
...
We create a configuration for the role ``redis/server`` in ``/srv/salt/redis/parameters/roles/redis/server.yaml``:
...
We create another configuration for the role ``redis/client`` in ``/srv/salt/redis/parameters/roles/redis/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles redis/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles redis/client
...
.. code-block:: console
    ...
    mv /srv/salt/redis/parameters/dns:domain/  '/srv/salt/redis/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-redis-as-json:
      test.nop:
        - name: {{ redis | json }}
...
Here is an example based on `template-formula/redis/config/file.sls`_:
...
This ``sls`` file expose a ``redis`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ redis['winner'] }}
...
.. _template-formula/redis/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/redis/config/file.sls
